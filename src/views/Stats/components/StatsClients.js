...
function StatsClients({ panel_content_className }) {
    ...
    const clientsAll = async () => {
        setLoading(true)
        const response = await getFAPI('stats/clientsAll')
        if (response.bool) setData(response.value)
        setLoading(false)
        setShowing('clientsAll')
    }
    ...
    return (
        <div className="flex flex-col gap-4">
            <div className={panel_content_className}>
                <Button
                    color={showing === 'clientsAll' ? 'success' : 'default'}
                    onClick={clientsAll}
                    isDisabled={loading}
                >
                    Todos
            ...
            <Tabs
                aria-label="estadisticas de clientes por categoria"
                variant="bordered"
                className="dark"
                classNames={{
                    base: "justify-center",
